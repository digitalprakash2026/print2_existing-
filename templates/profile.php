...
window.addEventListener('hashchange', () => {
  ...
  if (hashOrder) {
    sessionStorage.setItem('accountOpenOrder', hashOrder);
...
if (linkedOrder) {
  sessionStorage.setItem('accountOpenOrder', linkedOrder);
  sessionStorage.setItem('accountOpenTab', 'orders');
}
const initialAccountTab = getAccountTabFromHash() || ((linkedOrder || rememberedOrder) ? (sessionStorage.getItem('accountOpenTab') || 'orders') : 'dashboard');
...
document.querySelectorAll('.account-order-detail').forEach(detail => {
  detail.addEventListener('toggle', () => {
    ...
    if (sessionStorage.getItem('accountOpenOrder') === detail.dataset.orderDetail) {
      ...
      const detailHash = `#orders-${encodeURIComponent(detail.dataset.orderDetail || '')}`;
...
function restoreOpenAccountOrder(shouldScroll = false) {
  ...
  requestAnimationFrame(() => { restoringAccountOrder = false; });
}
