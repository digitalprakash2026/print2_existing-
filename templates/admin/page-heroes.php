...
<section class="adm-page-head page-heroes-head">
  <div>
    ...
    <h1>Page Heroes</h1>
    <p class="adm-muted">Manage background images and hero headlines used in the common breadcrumb/title section.</p>
  </div>
</section>
...
<section class="card page-heroes-card">
  <div class="page-heroes-toolbar">
    <div>
      ...
      <p>Upload a banner image and customize the headline/subtitle for each public page hero.</p>
    </div>
...
function heroCard(row){
  ...
  return `<article class="page-hero-admin-card" data-key="${esc(row.page_key)}">
    ...
    <div class="page-hero-admin-body">
      <h3>${esc(row.page_key || '')}</h3>
      <label>Hero Headline<input class="hero-title" value="${esc(row.title || '')}" placeholder="Hero headline"></label>
      <label>Hero Subtitle<textarea class="hero-desc" rows="2" placeholder="Short headline/subtitle below title">${esc(row.description || '')}</textarea></label>
      <label>Background Image Path<input class="hero-path" value="${esc(row.background_image || '')}" placeholder="Upload or paste image path"></label>
...
pageHeroList.addEventListener('click', async (e) => {
  ...
  const title = card.querySelector('.hero-title').value.trim();
  const description = card.querySelector('.hero-desc').value.trim();
  try{
    await api('/admin/api/page-heroes/' + encodeURIComponent(key), {method:'PUT', body:JSON.stringify({title, description, background_image, is_active:true})});
