...
async function loadCategories(){ const data=await api('/admin/api/portfolio-categories'); category_id.innerHTML='<option value="">Select product category</option>'+(data.categories||[]).map(c=>`<option value="${esc(c.id)}" data-name="${esc(c.name)}">${esc(c.name)}</option>`).join(''); }
