...
$pageTitle = 'Shop by Business Sector — RCS Graphic';
...
<main class="business-page all-business-page">
  ...
  <section class="shop-cat-section business-needs-section all-business-section" aria-labelledby="allBusinessTitle">
    <div class="shop-cat-container">
      <div class="shop-cat-head"><h2 class="shop-cat-title" id="allBusinessTitle">All <span>Business Sectors</span></h2><a href="/products" class="shop-cat-all">View All Products</a></div>
      ...
      <?php else: ?>
        <div class="business-sector-grid">
          <?php foreach ($businessNeeds as $idx => $need):
            ...
            $img = trim((string)($need['image_path'] ?? ''));
          ...
          <article class="shop-cat-card business-need-card" data-reveal data-reveal-delay="<?= ($idx % 4) * 50 ?>">
            <a href="<?= htmlspecialchars($href, ENT_QUOTES, 'UTF-8') ?>" class="shop-cat-link business-need-link">
              <div class="shop-cat-img business-need-img"><?php if ($img !== ''): ?><img src="<?= htmlspecialchars($img, ENT_QUOTES, 'UTF-8') ?>" alt="<?= htmlspecialchars($name, ENT_QUOTES, 'UTF-8') ?>" loading="lazy"><?php else: ?><div class="shop-cat-fallback" aria-hidden="true"><?= htmlspecialchars($icon, ENT_QUOTES, 'UTF-8') ?></div><?php endif; ?></div>
              <div class="shop-cat-body business-need-body compact"><span class="shop-cat-name"><?= htmlspecialchars($name, ENT_QUOTES, 'UTF-8') ?></span></div>
            </a>
          </article>
          <?php endforeach; ?>
        </div>
