...
if ($heroKey !== '') {
    try {
        $row = Database::row("SELECT title, description, background_image, fallback_image FROM page_heroes WHERE page_key=? AND is_active=1 LIMIT 1", [$heroKey]);
        $candidate = trim((string)($row['background_image'] ?? $row['fallback_image'] ?? ''));
        $dbTitle = trim((string)($row['title'] ?? ''));
